...
Verify in any mode with `pocsynth doctor` (CLI) or `./pocsynth.py --json doctor` (skill).
...
```bash
git clone <this-repo> && cd poc-synthetic-document-generator
uv tool install .
pocsynth --help
```
...
CI (`skill-script-drift` job) fails the MR if the committed artifact has drifted from source.
