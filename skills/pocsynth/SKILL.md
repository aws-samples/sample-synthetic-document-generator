...
description: >
  ...
  business description, or a reference document. Use when the user asks to
...
| `presets` | List bundled preset schemas — 15 record types (b2b_saas, ecommerce_orders, healthcare_lite, crm_contacts, insurance_claims, utility_meter, loyalty_pos, ad_campaign, knowledge_corpus, security_telemetry, support_tickets, commercial_leases, product_reviews, financial_transactions, contact_center_transcripts). All synthetic by construction → safe to share, no `verify` needed. | **free** |
...
| `ui` | Launch the demo web UI (requires `uv tool install '.[ui]'`). Each preview shows the equivalent `run` command in both CLI and `--json` agent-skill form, so the UI doubles as a way to learn these commands. | — |
